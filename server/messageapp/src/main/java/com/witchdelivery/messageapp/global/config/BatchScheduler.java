...
public class BatchScheduler {
    ...
    @Scheduled(cron = "0 0 0 * * *")      // 10초마다 실행되도록 예약.  매일 오전 09시 실행은, 0 0 9 * * *
    ...
    @Scheduled(cron = "0 0 0 * * *") // 밤 12시 실행
    public void runOutgoingJob() {
        Map<String, JobParameter> confMap = new HashMap<>();
        confMap.put("time", new JobParameter(System.currentTimeMillis())); // time과 밀리초 단위의 현재 시스템 시간 값을 사용하는 JobParameter 개체 생성 및 추가
        JobParameters jobParameters = new JobParameters(confMap);

        try {
            jobLauncher.run(batchConfig.outgoingJob(), jobParameters); // jobLauncher는 batchConfig.job()에서 지정한 작업을 실행하고 이전에 생성한 JobParameters 객체를 매개변수로 전달하는 데 사용된다.
        } catch (JobExecutionAlreadyRunningException | JobInstanceAlreadyCompleteException
                 | JobParametersInvalidException | org.springframework.batch.core.repository.JobRestartException e) {
    ...
    @Scheduled(cron = "0 0 0 * * *") // 밤 12시 실행
    public void runReceivingJob() {
        Map<String, JobParameter> confMap = new HashMap<>();
        confMap.put("time", new JobParameter(System.currentTimeMillis())); // time과 밀리초 단위의 현재 시스템 시간 값을 사용하는 JobParameter 개체 생성 및 추가
        JobParameters jobParameters = new JobParameters(confMap);

        try {
            jobLauncher.run(batchConfig.receivingJob(), jobParameters); // jobLauncher는 batchConfig.job()에서 지정한 작업을 실행하고 이전에 생성한 JobParameters 객체를 매개변수로 전달하는 데 사용된다.
        } catch (JobExecutionAlreadyRunningException | JobInstanceAlreadyCompleteException
                 | JobParametersInvalidException | org.springframework.batch.core.repository.JobRestartException e) {
